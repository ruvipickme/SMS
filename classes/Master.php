...
Class Master extends DBConnection {
    ...
    function save_workshop(){
        ...
        $check = $this->conn->query("SELECT * FROM `work_monitor` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
        if($this->capture_err())
            return $this->capture_err();
        if($check > 0){
            $resp['status'] = 'failed';
            $resp['msg'] = "Workshop Name already exist.";
            return json_encode($resp);
            exit;
        }
        if(empty($id)){
            $sql = "INSERT INTO `work_monitor` set {$data} ";
            $save = $this->conn->query($sql);
        }else{
            $sql = "UPDATE `work_monitor` set {$data} where id = '{$id}' ";
            $save = $this->conn->query($sql);
        }
        if($save){
            $resp['status'] = 'success';
            if(empty($id)){
                $res['msg'] = "New work monitor successfully saved.";
                $id = $this->conn->insert_id;
            }else{
                $res['msg'] = "Work monitor successfully updated.";
            }
            $this->settings->set_flashdata('success',$res['msg']);
        }else{
            $resp['status'] = 'failed';
    ...
    function delete_workshop(){
        extract($_POST);
        $del = $this->conn->query("DELETE FROM `work_monitor` where id = '{$id}'");
        if($del){
            $resp['status'] = 'success';
            $this->settings->set_flashdata('success',"Work monitor successfully deleted.");
        }else{
            $resp['status'] = 'failed';
            $resp['error'] = $this->conn->error;
        }
...
switch ($action) {
    case 'save_workshop':
        echo $Master->save_workshop();
        break;
    case 'delete_workshop':
        echo $Master->delete_workshop();
        break;
	case 'save_supplier':
